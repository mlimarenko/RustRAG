...
fn is_markdown_table_line(line: &str) -> bool {
    ...
    !parse_markdown_table_row(trimmed).is_empty()
}
...
#[cfg(test)]
mod tests {
    use super::{ExtractionLineSignal, infer_line_signals};

    #[test]
    fn single_column_markdown_table_lines_are_not_headings() {
        let header_signals = infer_line_signals("| col_1 |");
        let separator_signals = infer_line_signals("| --- |");
        let row_signals = infer_line_signals("| value |");

        assert!(header_signals.contains(&ExtractionLineSignal::TableRow));
        assert!(separator_signals.contains(&ExtractionLineSignal::TableRow));
        assert!(row_signals.contains(&ExtractionLineSignal::TableRow));

        assert!(!header_signals.contains(&ExtractionLineSignal::Heading));
        assert!(!separator_signals.contains(&ExtractionLineSignal::Heading));
        assert!(!row_signals.contains(&ExtractionLineSignal::Heading));
    }
}
