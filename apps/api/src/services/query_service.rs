...
use crate::{
    ...
    infra::{
        ...
        repositories::{self as graph_repo, ai_repository, query_repository, runtime_repository},
    },
...
impl QueryService {
    ...
    pub async fn get_execution(
        ...
        // Batch-fetch document metadata (title + source_uri) for segment references.
        let segment_document_ids: Vec<Uuid> = structured_block_rows
            .iter()
            .map(|b| b.document_id)
            .collect::<std::collections::HashSet<_>>()
            .into_iter()
            .collect();
        let segment_document_info: HashMap<Uuid, (Option<String>, Option<String>)> =
            if segment_document_ids.is_empty() {
                HashMap::new()
            } else {
                let docs = state
                    .arango_document_store
                    .list_documents_by_ids(&segment_document_ids)
                    .await
                    .unwrap_or_default();
                let doc_titles: HashMap<Uuid, Option<String>> =
                    docs.iter().map(|d| (d.document_id, d.title.clone())).collect();
                let revision_ids: Vec<Uuid> = structured_block_rows
                    .iter()
                    .map(|b| b.revision_id)
                    .collect::<std::collections::HashSet<_>>()
                    .into_iter()
                ...
                let revisions = state
                    ...
                    .await
                    .unwrap_or_default();
                let revision_source_uri: HashMap<Uuid, Option<String>> =
                    revisions.iter().map(|r| (r.document_id, r.source_uri.clone())).collect();
                segment_document_ids
                    .iter()
                    .map(|id| {
                        let title = doc_titles.get(id).cloned().flatten();
                        let uri = revision_source_uri.get(id).cloned().flatten();
                        (*id, (title, uri))
                    })
                    .collect()
            };
        ...
        let mut graph_node_references =
            bundle_refs.as_ref().map_or_else(Vec::new, map_entity_references);
...
fn build_prepared_segment_references(
    ...
    query_text: &str,
    document_info: &HashMap<Uuid, (Option<String>, Option<String>)>,
    ...
    let mut items = blocks
        ...
        .filter_map(|block| {
            ...
            let (doc_title, source_uri) =
                document_info.get(&block.document_id).cloned().unwrap_or_default();
...
async fn search_pg_entity_references(
    pool: &sqlx::PgPool,
    library_id: Uuid,
    execution_id: Uuid,
    query_text: &str,
