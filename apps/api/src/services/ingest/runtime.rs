...
pub async fn embed_runtime_graph_nodes(
    ...
    // future only borrows owned data and stays Send.
    type NodeBatch = Vec<(usize, String)>;
...
pub async fn embed_runtime_graph_edges(
    ...
    // Build owned (offset, input_text) batches up front so each parallel
    // future only borrows owned data and stays Send.
    type EdgeBatch = Vec<(usize, String)>;
